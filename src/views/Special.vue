<template>
    <div>
        <div style="margin: 20px 0">
            <b style="font-size: 18px; color: #323435;">特 殊 需 求 消 息 提 醒</b>
        </div>
        ...
        <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"
            ...
            <el-table-column prop="meetingDemand" label="特殊需求" align="center"></el-table-column>
            <el-table-column prop="status" label="确认状态" align="center"></el-table-column>
            <el-table-column label="操作" width="200" align="center">
                <template slot-scope="scope">
                    <el-popconfirm class="ml-5" confirm-button-text='确定' cancel-button-text='我再想想' icon="el-icon-info"
                        icon-color="green" title="您已确认好特殊需求了吗？" @confirm="yes(scope.row.id)">
        ...
        <el-dialog title="您确定要拒绝此特殊需求吗" :visible.sync="dialogRefuseVisible" width="35%">
...
export default {
    ...
    methods: {
        load() {
            this.request.get("msg/allDemands", {
                params: {
        ...
        yes(id) {
            this.request.get("/msg/demandSure?id=" + id).then(res => {
                if (res.code === 200) {
        ...
        refuse() {
            this.request.get(`/msg/demandDeny?id=${this.refuseForm.id}&reason=${this.refuseForm.reason}`)
                .then(res => {
                    if (res.code === 200) {
                        this.$message.success(res.msg)
                        this.load()
